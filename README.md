...
### Updating post
After making sure that the post author matches with the requested user post can be edited or deleted via fetching into this. one uses the PUT request method and the second one DELETE. To update the post, a post id is required with a new description and a new image provided. I tried to populate a new image file field value with the existing image but I only could set it equal to an empty string.
...
### Updating post like 
After adding all contents of the post to the card, post liking functionality kicks in. First of all, I designed it in a such way that like amount identificator and like logo are on the same line and if the like amount is equal to zero it isn't displaying at all. Then using click event listener on the like logo with an update like function fetch call is being run into this [URL](#update-post-like-postintpost_idupdate_like) and post like is being updated. Its response returns new amount and new status so with the use of that resources page reloading isn't required.

### Post comments
In the end, every post is having its own comments section, which is populated with comments after clicking on it. To make that happen post id is being sent via fetch call into this [URL](#comments-postintpost_idcomments). After getting post instance comments are being filtered based on that post, after receiving that response in JS, comments are appended to post at the bottom. Also, comment input is being displayed where the user can type a new comment and then save it. all this process runs without the requirement of refreshing the page. When a comment is created its dropdown is being added where are located comment edit and remove links. only its author can use this functionality. Also, comment has like icon where users can leave their reactions.

### Updating comment
I also added a dropdown menu for comments. If the comment author matches with the requested user they will have the ability to click on the dropdown button and then edit or delete a comment by sending comment id into [URL](#comment-postintcomment_id) and committing actions which were defined in python function.

### Comment likes
For comment liking defined update comment like function is being run. After successful fetch into this [URL](#update-comment-like-commentintcomment_idupdate_like) like is added or removed in dependence of resulting if statement and all data that was being returned from the response is used to update comment-state so there will be no requirement for the page to refresh. if a comment likes amount is equal to zero it will not be displayed.
...
### Profile
The user profile is being shown when clicking on post author username (by default first page of paginator is displayed), this process is feasible by fetching into [profile URL](#profile-profileintprofile_id) with specified profile id. When unhiding profile with including data from response new post form disappears. In contrast to all posts page profile page only shows posts related to the profile which is being displayed.
